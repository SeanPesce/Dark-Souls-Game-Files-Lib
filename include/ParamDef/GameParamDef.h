/*
	...
	GameFiles/ParamDef/GameParamDef.h
...
protected:
	...
	void *init(void **start, const char *aob, bool print_result = false, bool re_init = false)
	{
		...
		if ((*start) != NULL && print_result)
		{
			...
			print_console(std::string("    Found param file: " + this->file + this->param_extension + " (Location: 0x").append(hex_stream.str()).append(") containing " + std::to_string(param_count) + " parameters"));
		}
