/*
	...
	GameFiles/Archive/Bhd5Archive.h
...
#ifndef _DS1_OVERHAUL_BHD5_ARCHIVE_FILE_H_
	#define _DS1_OVERHAUL_BHD5_ARCHIVE_FILE_H_
...
public:
	...
	// BHD5 file extension
	static const char    *FILE_EXTENSION;   //  ".bhd5" (char)
	static const wchar_t *FILE_EXTENSION_W; // L".bhd5" (wide char)
	...
	*/
	int to_file(const char *file, bool overwrite = true)
	{
		...
		return Files::dump_file(_header, _header->file_size, file, overwrite);
	}
	...
	*/
	void *extract_from_bdt_by_index(const char *bdt_file, void *extract_buffer, uint32_t bin_index, unsigned int record_index)
	{
		...
		return Files::read_from_offset(bdt_file, record->offset, record->size, extract_buffer, true);
	}
	...
	*/
	int extract_from_bdt_by_index(const char *bdt_file, uint32_t bin_index, unsigned int record_index, const char *write_file, bool overwrite = true)
	{
		...
		if ((extracted_file_in_mem = Files::read_from_offset(bdt_file, record->offset, record->size, NULL, true)) == NULL)
			return GetLastError();

		int return_val = Files::dump_file(extracted_file_in_mem, record->size, write_file, overwrite);
	...
	*/
	static void *file_to_mem(const char *bhd5_file, void *file_buffer)
	{
		...
		if (!Files::file_exists(bhd5_file))
		{
	...
	*/
	static Bhd5Bin *bin_from_file_by_index(const char *bhd5_file, Bhd5Bin *bin_buffer, uint32_t bin_index, bool check_bin_index = true)
	{
		...
		return (Bhd5Bin*)Files::read_from_offset(bhd5_file, sizeof(Bhd5Header) + (bin_index * sizeof(Bhd5Bin)), sizeof(Bhd5Bin), bin_buffer);
	}
	...
	static Bhd5Bin *bin_from_file_by_hash(const char *bhd5_file, Bhd5Bin *bin_buffer, uint32_t hash)
	{
		...
		if(found)
			return (Bhd5Bin*)Files::read_from_offset(bhd5_file, sizeof(Bhd5Header) + (i * sizeof(Bhd5Bin)), sizeof(Bhd5Bin), bin_buffer);
		// Failed to find a record with the given hash
		SetLastError(ERROR_SECTOR_NOT_FOUND);
		return NULL;
	}
	...
	*/
	static Bhd5Record *record_from_file_by_index(const char *bhd5_file, Bhd5Record *record_buffer, uint32_t bin_index, unsigned int record_index, bool check_bin_index = true)
	{
		...
		if (!(Bhd5Bin*)Files::read_from_offset(bhd5_file, sizeof(Bhd5Header) + (bin_index * sizeof(Bhd5Bin)), sizeof(Bhd5Bin), &bin))
			return NULL; // Error reading file
		...
		return (Bhd5Record*)Files::read_from_offset(bhd5_file, bin.record_offset + (record_index * sizeof(Bhd5Record)), sizeof(Bhd5Record), record_buffer);
	}
	...
	static Bhd5Record *record_from_file_by_hash(const char *bhd5_file, Bhd5Record *record_buffer, uint32_t hash)
	{
		...
		if(found)
			return (Bhd5Record*)Files::read_from_offset(bhd5_file, bin.record_offset + (j * sizeof(Bhd5Record)), sizeof(Bhd5Record), record_buffer);
		// Failed to find a record with the given hash
		SetLastError(ERROR_SECTOR_NOT_FOUND);
		return NULL;
	}
	...
	*/
	static Bhd5Record *bin_records_from_file_by_index(const char *bhd5_file, Bhd5Record *record_buffer, uint32_t bin_index, bool check_bin_index = true)
	{
		...
		return (Bhd5Record*)Files::read_from_offset(bhd5_file, bin.record_offset, (sizeof(Bhd5Record) * bin.record_count), record_buffer);
	}
...
#endif // _DS1_OVERHAUL_BHD5_ARCHIVE_FILE_H_