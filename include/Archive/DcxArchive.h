/*
	...
	GameFiles/Archive/DcxArchive.h
...
#ifndef _DS1_OVERHAUL_DCX_ARCHIVE_FILE_H_
	#define _DS1_OVERHAUL_DCX_ARCHIVE_FILE_H_
...
public:
	// DCX file extension
	static const char    *FILE_EXTENSION;   //  ".dcx" (char)
	static const wchar_t *FILE_EXTENSION_W; // L".dcx" (wide char)
	...
	uint32_t subheader_size() { if(_header == NULL){SetLastError(ERROR_CANNOT_MAKE); return 0;} return Files::reverse_endian(_header->subheader_size_big_endian); }
	uint32_t size_decompressed() { if(_header == NULL){SetLastError(ERROR_CANNOT_MAKE); return 0;} return Files::reverse_endian(_header->size_decompressed_big_endian); }
	uint32_t size_compressed() { if(_header == NULL){SetLastError(ERROR_CANNOT_MAKE); return 0;} return Files::reverse_endian(_header->size_compressed_big_endian); } // Size of the compressed data ONLY
	...
	uint32_t compression_header_size() { if(_header == NULL){SetLastError(ERROR_CANNOT_MAKE); return 0;} return Files::reverse_endian(_header->compression_header_size_big_endian); }
	...
	*/
	int to_file(const char *new_file, bool overwrite = true)
	{
		...
		// Write file
		return Files::dump_file(_header, file_size(), new_file, overwrite);
	}
	...
	*/
	int extract_and_dump(const char *new_file, bool free_memory = true, bool overwrite = true)
	{
		...
		// Write file
		return Files::dump_file(saved_extract_data, size_decompressed(), new_file, overwrite);
	}
	...
	{
		uint32_t size = Files::reverse_endian(((DcxHeader*)new_file_start)->size_compressed_big_endian) + sizeof(DcxHeader);
...
#endif // _DS1_OVERHAUL_DCX_ARCHIVE_FILE_H_